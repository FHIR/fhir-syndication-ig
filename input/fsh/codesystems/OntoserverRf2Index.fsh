...
On the wire, the Atom `<category scheme="…">` attribute carries one
...
The label paired with this term in `<category>` is conventionally
`Binary Index`.
"""
...
* #BINARY "Binary Index" "A pre-built Ontoserver RF2 binary index — a zip containing the on-disk index Ontoserver derives from a SNOMED CT RF2 release. The companion `<category scheme>` URI carries the binary-format version."
