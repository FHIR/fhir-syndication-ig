...
* link 1..* SyndicationLink "Atom `<link>`. At least one link MUST be present. The primary artefact uses `rel=\"alternate\"`; ancillary documents (release notes, licence) use `rel=\"related\"`."
...
* fhirVersion 0..1 string "`ncts:fhirVersion`. The FHIR version the artefact targets, when the artefact is FHIR. Format: `<major>.<minor>` or `<major>.<minor>.<patch>` (e.g. `4.0.1`). Maximum length 5. MUST be present when at least one category has a `FHIR_*` term."
...
* bundleInterpretation 0..1 code "`ncts:bundleInterpretation`. For artefacts that are FHIR Bundles: the intended load semantics. Permitted values: `batch`, `collection`. When absent for a Bundle artefact, consumers SHOULD default to `batch`."
...
* packageDependency 0..1 BackboneElement "`sct:packageDependency`. Inter-package dependencies. Present only when the artefact requires other SNOMED CT packages (i.e. is an extension or derivative — not for an Edition itself)."
